...
class Map 
{
	...
	static public function loadFromName(name:String):Map {
		return loadFromByteArray(Decrypt.decryptDataWithKey(BraveAssets.getBytes(Std.format("map/$name.dat")), Decrypt.key23));
	}

	static public function loadFromByteArray(data:ByteArray):Map {
		...
		//Log.trace(Std.format("$width,$height"));
		...
		for (y in 0 ... height) {
			for (x in 0 ... width) {
				...
				//if (y > 70) Log.trace(Std.format("$x,$y : ${data.bytesAvailable}"));
