...
version (Windows) {
	...
	private void glInitWindows() {
		...
		if (!opengl_dll) throw(new Exception("Can't load opengl32.dll"));
		...
		mixin(glBindFuncBaseMix("wglSwapIntervalEXT"));
	}
} else {
	static assert (0 != 0);
}
...
// Used for API == 1.1
void gluBindFuncBase(void** ptr, char[] funcName) {
	version (Windows) {
		void* func = GetProcAddress(glu_dll, std.string.toStringz(funcName));
		if (!func) throw(new Exception(std.string.format("Can't bind '%s'", funcName)));
		*ptr = func;
	} else {
		throw(new Exception(std.string.format("Can't bind '%s' (only works on windows)", funcName)));
...
char[] gluBindFuncMix(char[] t) { return "gluBindFuncBase(cast(void**)&" ~ t ~ ", \"" ~ t ~ "\");"; }



void gluInitSystem() {
	glu_dll = LoadLibraryA("glu32.dll");
	
	if (!glu_dll) throw(new Exception("Can't load glu32.dll"));
	
	mixin(gluBindFuncMix("gluBeginCurve"));
	mixin(gluBindFuncMix("gluBeginPolygon"));
	mixin(gluBindFuncMix("gluBeginSurface"));
	mixin(gluBindFuncMix("gluBeginTrim"));
	//mixin(gluBindFuncMix("gluBuild1DMipmapLevels"));
	mixin(gluBindFuncMix("gluBuild1DMipmaps"));
	//mixin(gluBindFuncMix("gluBuild2DMipmapLevels"));
	mixin(gluBindFuncMix("gluBuild2DMipmaps"));
	//mixin(gluBindFuncMix("gluBuild3DMipmapLevels"));
	//mixin(gluBindFuncMix("gluBuild3DMipmaps"));
	//mixin(gluBindFuncMix("gluCheckExtension"));
	mixin(gluBindFuncMix("gluCylinder"));
	mixin(gluBindFuncMix("gluDeleteNurbsRenderer"));
	mixin(gluBindFuncMix("gluDeleteQuadric"));
	mixin(gluBindFuncMix("gluDeleteTess"));
	mixin(gluBindFuncMix("gluDisk"));
	mixin(gluBindFuncMix("gluEndCurve"));
	mixin(gluBindFuncMix("gluEndPolygon"));
	mixin(gluBindFuncMix("gluEndSurface"));
	mixin(gluBindFuncMix("gluEndTrim"));
	mixin(gluBindFuncMix("gluGetNurbsProperty"));
	mixin(gluBindFuncMix("gluGetTessProperty"));
	mixin(gluBindFuncMix("gluLoadSamplingMatrices"));
	mixin(gluBindFuncMix("gluLookAt"));
	mixin(gluBindFuncMix("gluNextContour"));
	mixin(gluBindFuncMix("gluNurbsCallback"));
	//mixin(gluBindFuncMix("gluNurbsCallbackData"));
	//mixin(gluBindFuncMix("gluNurbsCallbackDataEXT"));
	mixin(gluBindFuncMix("gluNurbsCurve"));
	mixin(gluBindFuncMix("gluNurbsProperty"));
	mixin(gluBindFuncMix("gluNurbsSurface"));
	mixin(gluBindFuncMix("gluOrtho2D"));
	mixin(gluBindFuncMix("gluPartialDisk"));
	mixin(gluBindFuncMix("gluPerspective"));
	mixin(gluBindFuncMix("gluPickMatrix"));
	mixin(gluBindFuncMix("gluProject"));
	mixin(gluBindFuncMix("gluPwlCurve"));
	mixin(gluBindFuncMix("gluQuadricCallback"));
	mixin(gluBindFuncMix("gluQuadricDrawStyle"));
	mixin(gluBindFuncMix("gluQuadricNormals"));
	mixin(gluBindFuncMix("gluQuadricOrientation"));
	mixin(gluBindFuncMix("gluQuadricTexture"));
	mixin(gluBindFuncMix("gluScaleImage"));
	mixin(gluBindFuncMix("gluSphere"));
	mixin(gluBindFuncMix("gluTessBeginContour"));
	mixin(gluBindFuncMix("gluTessBeginPolygon"));
	mixin(gluBindFuncMix("gluTessCallback"));
	mixin(gluBindFuncMix("gluTessEndContour"));
	mixin(gluBindFuncMix("gluTessEndPolygon"));
	mixin(gluBindFuncMix("gluTessNormal"));
	mixin(gluBindFuncMix("gluTessProperty"));
	mixin(gluBindFuncMix("gluTessVertex"));
	mixin(gluBindFuncMix("gluUnProject"));
	//mixin(gluBindFuncMix("gluUnProject4"));
	mixin(gluBindFuncMix("gluErrorString"));
	mixin(gluBindFuncMix("gluErrorUnicodeStringEXT"));
	mixin(gluBindFuncMix("gluGetString"));
	mixin(gluBindFuncMix("gluNewNurbsRenderer"));
	mixin(gluBindFuncMix("gluNewQuadric"));
	mixin(gluBindFuncMix("gluNewTess"));	
}
...
class GLControl : Control {
	...
	override protected void onHandleCreated(EventArgs ea) {
		try {
			...
			writefln("GLControl.onHandleCreated() : %s", e.toString);
		}
    ...
    override void onHandleDestroyed(EventArgs ea) {
		try {
			...
			writefln("GLControl.onHandleDestroyed() : %s", e.toString);
		}
    ...
    protected void setupPixelFormat() {
		try {
			...
			writefln("GLControl.setupPixelFormat() : %s", e.toString);
		}
    ...
    override void onResize(EventArgs ea) {
		...
		try {
			...
			writefln("GLControl.onResize() : %s", e.toString);
		}
    }
    ...
    protected void createContext() { _context = new GLContext(this,_context); }
}
